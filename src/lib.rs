...
pub struct VSD<T: 'static + serde::Serialize + serde::de::DeserializeOwned, LOCK: ?Sized> {
...
impl <T: serde::Serialize + serde::de::DeserializeOwned, LOCK: ?Sized> VSD<T, LOCK> {
...
impl <T: serde::Serialize + serde::de::DeserializeOwned, VSDLocked: ?Sized>Drop for VSD<T, VSDLocked> {
    fn drop(&mut self) {
        println!("Dropping.");
    }
}

impl <T: serde::Serialize + serde::de::DeserializeOwned + Send + Sync, LOCK: VSDLocked + Send>VSDLocked for VSD<T, LOCK> {
    /// Opens and reads a database from file.
